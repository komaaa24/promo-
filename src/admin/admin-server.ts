...
function dashboardHtml(): string {
  ...
  *{box-sizing:border-box} body{margin:0;background:#f5f6f8;color:#20242a;font-family:Arial,sans-serif} .app{display:grid;grid-template-columns:260px 1fr;min-height:100vh}
  ...
  const fmt=n=>Number(n||0).toLocaleString('ru-RU'); const esc=v=>String(v??'').replace(/[&<>"]/g,s=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[s])); const qs=()=>new URLSearchParams({q:el('q').value,region:el('region').value,from:el('from').value,to:el('to').value}).toString();
  ...
  async function participantsView(){const d=await api('/dashboard/api/participants?'+qs()); el('participants').innerHTML='<div class="panel"><b>Jami: '+fmt(d.total)+'</b></div>'+table(d.rows)}
  async function codesView(){const d=await api('/dashboard/api/codes?'+qs()); el('codes').innerHTML='<div class="panel"><b>Jami: '+fmt(d.total)+'</b><div class="hint">Kodlar xavfsizlik uchun faqat oxirgi 4-8 belgisi bilan korsatiladi.</div></div>'+table(d.rows)}
